  ...
  const save = () => {
    ...
    setTimeout(() => {
      try {
        ...
        const statusLabel = status === 'empty' ? 'Empty' : status === 'half' ? 'Filling' : 'Full';
        toast({ 
  ...
  return (
    <div className="space-y-6 animate-fade-in">
        ...
        <div className="space-y-2">
          {(['empty', 'half', 'full'] as const).map((occ) => {
            const labels = {
              ...
              half: '🚗 Filling - Some Seats Available',
      ...
      <div className={cn(
        ...
        <p className="text-lg">{status === 'empty' ? '🚕 Empty' : status === 'half' ? '🚗 Filling' : '🚙 Full'}</p>
      </div>
